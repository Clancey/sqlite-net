...
namespace SQLite.Providers
{
	internal class SqlServerProvider : IDatabaseProvider
	{
		...
		public bool SupportsInsertOrReplace => false; // SQL Server uses MERGE instead
		...
		public bool CanHandleConnectionString(string connectionString)
		{
			...
			// Avoid conflicts with MariaDB and Azure SQL
			if (connectionString.Contains("Provider=MariaDB") || connectionString.Contains("Provider=AzureSql"))
				return false;
				
			return connectionString.Contains("Data Source=") && connectionString.Contains("Initial Catalog=") ||
				   connectionString.Contains("Integrated Security=") ||
				   connectionString.Contains("Trusted_Connection=") ||
				   connectionString.Contains("MultipleActiveResultSets=");
		}
		...
		public string TransformConnectionString(string connectionString)
		{
			...
			// Remove Provider=SqlServer parameter
			if (result.Contains("Provider=SqlServer"))
			{
				result = result.Replace("Provider=SqlServer;", "").Replace(";Provider=SqlServer", "").Replace("Provider=SqlServer", "");
			}
		...
		{
			// SQL Server doesn't support virtual tables, and IF NOT EXISTS isn't directly supported
		...
		{
			// SQL Server doesn't support WITHOUT ROWID
			return "";
		}
		
		public bool SupportsIfNotExists => false;
		
		// CRUD SQL Generation - SQL Server specific implementations
		public string GenerateInsertSql(TableMapping table, string extra)
		{
			...
			else
			{
				if (replacing && !SupportsInsertOrReplace)
				{
					...
					var pk = table.PK;
					if (pk == null)
						throw new NotSupportedException("InsertOrReplace requires a primary key in SQL Server");
...
WHEN NOT MATCHED THEN
	INSERT ({5}) VALUES ({6});",
		...
		// Transaction support - SQL Server syntax
		...
		{
			// SQL Server doesn't have RELEASE SAVEPOINT, savepoints are automatically released
		...
		public void ConfigureConnection(IDbConnection connection)
		{
			...
			if (connection is SqlConnection sqlConn)
			{
				...
				// Note: SqlConnection doesn't have a settable timeout property
				// Timeout is controlled by the connection string
			}
		}
	}
}